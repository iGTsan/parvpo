        ...
        [&](const crow::request& req, crow::response& res) {
            ...
            if (prod_type == 1) {
                if (prod_data == -1) {
                    ...
                    if (matrix1_demo.size() % 1000 == 0) {
                        // std::time_t now_time = std::chrono::system_clock::to_time_t(std::chrono::system_clock::now());
                        // std::cout << matrix1_demo.size() << " message sended for mat1" << std::ctime(&now_time) << std::endl;
                    }
                }
            } 
            else if (prod_type == 2) {
                if (prod_data == -1) {
                    ...
                    if (matrix2_demo.size() % 1000 == 0) {
                        // std::time_t now_time = std::chrono::system_clock::to_time_t(std::chrono::system_clock::now());
                        // std::cout << matrix2_demo.size() << " message sended for mat2" << std::ctime(&now_time) << std::endl;
                    }
                }
            } 
            else {
            ...
            if (got_msg % 1000 == 0) {
                std::time_t now_time = std::chrono::system_clock::to_time_t(std::chrono::system_clock::now());
                std::cout << got_msg << " message recieved " << std::ctime(&now_time) << std::endl;
            }
